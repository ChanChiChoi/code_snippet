...
//--------------------------------
__global__ void
copy_const_kernel(float *iptr, float const *cptr){
  int x = threadIdx.x+blockIdx.x*blockDim.x;
  int y = threadIdx.y+blockIdx.y*blockDim.y;
  int offset = x+y*blockDim.x*gridDim.x;
...
__global__ void
blend_kernel(float *outSrc, float const *inSrc){
  int x = threadIdx.x+blockIdx.x*blockDim.x;
  int y = threadIdx.y+blockIdx.y*blockDim.y;
  int offset = x+y*blockDim.x*gridDim.x;
...
void anim_gpu(DataBlock *d, int ticks){
  ...
  Mat *bitmap = d->bitmap;
  for(int i=0; i<90; i++){
    copy_const_kernel<<<grid, threads>>>(d->dev_inSrc, d->dev_constSrc);
    blend_kernel<<<grid,threads>>>(d->dev_outSrc, d->dev_inSrc);
    // 指针交换
    auto tmp = d->dev_outSrc;
    d->dev_outSrc = d->dev_inSrc;
    d->dev_inSrc = tmp;
  }
...
int main(int argc, char *argv[]){
  ...
  for(int i=0; i<190; i++){
    anim_gpu(&data,1);
//    checkCudaErrors(cudaMemcpy(bitmap.data,data.output_bitmap,bitmap.elemSize()*bitmap.total(),
//                              cudaMemcpyDeviceToHost));
