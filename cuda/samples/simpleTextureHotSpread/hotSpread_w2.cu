...
int main(int argc, char *argv[]){
  ...
  checkCudaErrors(cudaBindTexture2D(NULL, texConstSrc, data.dev_constSrc,
                                    desc,DIM,DIM, sizeof(float)*DIM));
  checkCudaErrors(cudaBindTexture2D(NULL, texIn, data.dev_inSrc,
                                    desc,DIM,DIM, sizeof(float)*DIM));
  checkCudaErrors(cudaBindTexture2D(NULL, texOut, data.dev_outSrc,
                                    desc, DIM, DIM, sizeof(float)*DIM));
